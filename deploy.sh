...
echo "Code files: engine/ server/ ui/ session/ app/ index.html package.json main.js styles/"
if $PUSH_ASSETS; then
...
scp $SSH_OPTS -r engine server ui session app index.html package.json main.js styles "$SERVER:$REMOTE_DIR/" 2>&1
