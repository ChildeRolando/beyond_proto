...
export const SKILLS = {
  ...
  mage_realm_sweep: {
    ...
    desc: '自身半径2范围内生成静止AOE弹体，对范围内所有敌人造成伤害。\n范围：半径2；威力：700；速度：1；费用：气7',
  },
  ...
  mage_reactive: {
    ...
    desc: '自身半径1范围内7格生成静止弹体，每个弹体威力等于当前护盾值。\n范围：半径1（自身中心）；威力：等于当前护盾值；速度：1；费用：无',
  },
  ...
  mage_armor_breaker: {
    ...
    desc: '生成直线飞行弹体，无限射程；弹体命中后清空目标所有气/怒/弹。\n范围：无限；威力：0；速度：1；CD：0；费用：气2',
  },
  ...
  mage_sword_flight: {
    ...
    desc: '向目标方向持续移动2回合（永久状态）；撞击敌人造成300伤害（无视减伤）；剑体可吸收300伤害后被摧毁。\n范围：方向1；威力：撞击300（无视减伤）；速度：2；费用：气3',
  },
  ...
  mage_formation: {
    ...
    desc: '目标格及半径1范围内设为阵法区域；阵法能量300，吸收范围内友方受到的伤害；阵眼被击中时阵法破灭。\n范围：5；威力：无；速度：1；费用：气3',
  },
  ...
  warrior_dash: {
    ...
    desc: '位移至目标相邻格；终点半径1范围内自动斩击（优先角色，其次弹体）；命中时获得1点怒气。\n范围：1；威力：100；速度：1；费用：怒气1',
  },
  ...
  warrior_sheathe: {
    ...
    desc: '获得纳刀状态，自动拦截范围内弹体（威力300）；成功斩破弹体后获得[引刀]：刷新居合斩CD并使下次居合斩cost=0。\n范围：自身；威力：反击300；速度：3；费用：无；冷却：2',
  },
  ...
  warrior_feint: {
    ...
    desc: '2速阶段背离目标退1格，0速阶段向目标方向冲刺2格，斩击冲刺路径上所有敌人；命中时获得1点怒气。\n范围：1；威力：100（路径AOE）；速度：2；费用：怒气1',
  },
  ...
  warrior_iaido: {
    ...
    desc: '范围4的高速斩击；若拥有[引刀]状态则费用变为0且刷新冷却；命中时获得1点怒气。\n范围：4；威力：100；速度：1；费用：怒气3；冷却：4',
  },
  ...
  warrior_lock: {
    ...
    desc: '标记敌方目标；下回合根据目标是否使用移动技能产生分支效果。\n范围：无限；威力：无；速度：1；费用：0',
  },
  ...
  warrior_blink_strike: {
    ...
    desc: '传送至5范围内目标背后位置，然后进行近战斩击；命中时获得1点怒气。对被追猎目标无视距离限制且不消耗冷却。\n范围：5（对被追猎目标无视距离）；威力：100；速度：1；CD：6；费用：怒气3',
  },
  ...
  warrior_meteor: {
    ...
    desc: '获得陨星升空状态（持续1回合，升空期间无敌）；下回合2速阶段自动在目标格坠落并造成AOE伤害。\n范围：8；威力：坠落AOE 500；速度：2；费用：怒气7',
  },
  ...
  warrior_meteor_resolve: {
    ...
    desc: '在陨星标记位置坠落，对半径1范围内敌人造成500伤害。此技能由系统自动执行，不可主动选择。\n范围：自身；威力：500（AOE半径1）；速度：2；费用：无',
  },
  ...
  shooter_bell: {
    ...
    desc: '消耗当前所有弹药；获得丧钟待发状态（持续1回合）；下回合自动对目标射出必中弹体，弹体数量等同于消耗的弹药量。\n范围：6；威力：100×消耗弹药数；速度：1；费用：消耗全部弹药',
  },
  ...
  shooter_predict: {
    ...
    desc: '对6范围内目标施加必中标记（持续1回合）；下回合对该目标的攻击必定命中。\n范围：6；威力：无；速度：1；费用：无',
  },
  ...
  shooter_slow_shot: {
    ...
    desc: '生成直线飞行弹体，射程6；命中后对目标施加定身状态（持续2回合）；弹壳掉落。\n范围：6；威力：100；速度：1；费用：弹药2',
  },
  ...
  shooter_armor_pierce: {
    ...
    desc: '生成直线飞行弹体，射程6；弹体具有穿甲效果（穿透护盾、格挡、防御阵符）；弹壳掉落。\n范围：6；威力：100（穿甲）；速度：1；费用：弹药2',
  },
  ...
  shooter_gun_dance: {
    ...
    desc: '自身半径2范围内连续生成4波静止AOE弹体，每波掉落1个弹壳。\n范围：半径2；威力：100×4，总计400；速度：1；费用：弹药4',
  },
  ...
  shooter_causality: {
    ...
    desc: '生成直线飞行弹体，射程10；具备穿甲效果，可在次元中造成伤害；弹壳掉落。\n范围：10；威力：1000（穿甲）；速度：1；费用：弹药6',
  },
  ...
  role_jimmy_marrow_wine: {
    ...
    desc: '增加洗髓层数；层数越高后续饮酒费用越高；依次获得5层强化（怒气获得+1/攻击距离+1/移动和饮酒视为灵巧/怒气获得+1/威力+100）。\n范围：自身；威力：无；速度：1；费用：怒气按层数递增，3/4/4/5/5',
  },
  role_duelist_windstep: {
    ...
    desc: '位移2格至目标位置；终点半径1范围内自动斩击（优先角色，其次弹体）；命中时获得1点怒气。冷却3回合。\n范围：2；威力：100；速度：1；费用：怒气1',
  },
  ...
  role_helldiver_supply_drop: {
    ...
    desc: '目标格空投降落补给箱；可被角色拾取，角色拾取后背包弹药+3。\n范围：6；威力：无；速度：1；费用：无',
  },
  role_helldiver_bombardment: {
    ...
    desc: '标记目标位置并获得轰炸待发状态（持续1回合）；下回合自动发射弹体。\n范围：6；威力：100（下回合结算）；速度：1；费用：无',
  },
  ...
  role_yan_empty_gun: {
    ...
    desc: '对6范围内目标施加标记状态（持续至本回合结束）；若本回合目标未发起攻击，则剥夺其全部资源。每场战斗仅限使用1次。\n范围：6；威力：无；速度：3；费用：无',
  },
  ...
  trait_jimmy_breathing: {
    ...
    desc: '奇数回合进入"吸"状态（盛怒额外获得1怒气，攻击距离-1）；偶数回合进入"呼"状态（攻击距离+1，盛怒少获得1怒气最低0）。不影响命中回怒或其他怒气来源。',
  },
  trait_jimmy_marrow: {
    ...
    desc: '回合结束时若怒气达标（5/6/7/8/9），自动扣除怒气并获得永久强化（依次为怒+1/攻击距离+1/移动视为灵巧/怒+1/威力+100）。',
  },
  trait_gunfighter_finesse: {
    ...
    desc: '每两回合获得一个灵巧行动点，可额外提交一个费用为0的行动，不占用主行动点数。',
  },
  ...
  trait_yan_death_wind: {
    ...
    desc: '每当对手发起攻击但未命中时，获得1子弹并立即执行一次上膛（不占用行动）。',
  },
  ...
  trait_helldiver_priority_ready: {
    ...
    desc: '呼叫类技能（轰炸、补给）延迟-1回合，即本回合提交后当回合立即结算。',
  },
  trait_helldiver_fast_ready: {
    ...
    desc: '呼叫技能+50技能急速，冷却时间从6回合缩短为4回合。',
  },
  trait_helldiver_speed_draw: {
    ...
    desc: '呼叫类技能视为灵巧行动，每回合可使用无限个灵巧呼叫行动点。',
  },
  ...
  trait_mirror_dimension_child: {
    ...
    desc: '角色独自存在于次元中时获得次元token（机制暂未实装）。',
  },
  trait_mirror_dimension_lord: {
    ...
    desc: '积累次元token后解锁次元系永久强化（机制暂未实装）。',
  },
  trait_mirror_phase_sling: {
    ...
    desc: '技能弹体穿过次元门后获得加速和威力提升（机制暂未实装）。',
  },
  trait_duelist_minds_eye: {
    ...
    desc: '所有敌人随机2个方向有弱点标记；弱点方向击中回复1怒且逐风步CD-1；命中后弱点刷新。',
  },
